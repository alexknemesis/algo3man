...
public abstract class Fantasma extends Personaje {
	...
	public void vivir() {
		...
		// Esto es una versi贸n primitiva de la implementaci贸n del contador
		// de tics, pero igual no creo que tenga mucha m谩s vuelta..
	...
	private void determinarEstado() {
		//Cambia el estado solo si sali贸 del modo disperso
		if(this.contadorTicsParaSalirDeDisperso == this.ticsParaSalirDeDisperso){
	 ...
	 * (paso m茅todos de caza a protected)
	 */
	...
	protected int direccionParaMinimaDistanciaA(Objetivo objetivo){
		...
		//Inicializo la variable con la m谩xima distancia que puede existir en el juego
		distanciaMinima = this.juego.getDimensiones().distance(new Point(0,0));

		direccionAMinimaDistancia = -1;
		
		celdasAlrededor = new Celda[4];
		//Cambie esto porque ya hab韆 un getCelda pero de Points y estaba mal pedida la posicion izq y der
		...
		if (direccionAMinimaDistancia != -1){
			return direccionAMinimaDistancia; //Encontr贸 una direcci贸n nueva
		}
		return direccionContraria(this.direccion); //Encontr贸 una pared y debe dar vuelta
	}
	...
	protected int direccionParaMaximaDistanciaA(Objetivo objetivo){
		...
		if (direccionAMaximaDistancia != -1){
			return direccionAMaximaDistancia; //Encontr贸 una direcci贸n nueva
		}
		return direccionContraria(this.direccion); //Encontr贸 una pared y debe dar vuelta
	}
